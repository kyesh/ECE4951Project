...
static void findSquares( const Mat& image, vector<vector<Point> >& p_squares )
{
    p_squares.clear();//clear the squares array
	...
	timg= cv::Mat::zeros(image.size(), image.type());	
	image.copyTo(timg);
    ...
    {
        int ch[] = {c, 0};
        int N = 1;//Number of levels for edge detection
        int thresh = 500;
	...
	//gray0=destination array, 1=number of matricies in gray0
	    ...
	    printf("Number of Countours Found = %d\n", contours.size());

		cv::namedWindow("Found Countors", cv::WINDOW_NORMAL );
                cv::imshow("Found Countors", gray);
...
static void drawSquares( Mat& image, const vector<vector<Point> >& p_squares )
{
	printf("Number of squares found = %d\n",p_squares.size());
    for( size_t i = 0; i < p_squares.size(); i++ )
    {
        const Point* p = &p_squares[i][0];
        int n = (int)p_squares[i].size();
...
int main(int argc, char** argv )
{
    ...
    cv::Mat img, img_resize, img_w, img_g, img_gE, img_gD1, img_gD2, img_squares;
    DIR *dir;
...
//                greenPassFilter[b][g][r] = (uchar)computeGreeness(b,g,r);
    ...
    if(dir != NULL){
	    ...
	    //cv::resize(img,img_resize, cv::Size(480,360), 0, 0, interpolation);
...
//                string outputPath(outdirectory + "SomeQualifier_"  + imgName);
		...
		cv::waitKey(0);//Wait until user presses key to continue
                findSquares(img_g, squares);
		

		img_squares= cv::Mat::zeros(img.size(), img.type());
		
                drawSquares(img_squares, squares);
		cv::namedWindow("Found Squares", cv::WINDOW_NORMAL );
		cv::imshow("Found Squares", img_squares);
		...
		cv::imwrite(outdirectory + "DetectedSquares_"  + imgName, img_squares);
